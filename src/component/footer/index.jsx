        ...
        {isMobile ? (
          <div className="flex flex-col justify-start items-start gap-4 w-full">
            <div className="flex flex-col justify-start items-start gap-4 w-full">
              ...
              <div className='cursor-pointer w-full bg-opacity-0 flex flex justify-between '  onClick={() => setShowGenre(!showGenre)}>
            ...
            <div className="flex flex-col justify-start items-start gap-4 w-full">
              ...
              <div className='cursor-pointer w-full bg-opacity-0 flex flex justify-between '  onClick={() => setShowBantuan(!showBantuan)}>
                <span className="text-white">Bantuan</span>
                <IoIosArrowForward className='text-white' />
              </div>
              {showBantuan && (
                <div className="grid grid-cols-1 gap-4 text-white">
                  {bantuan.map((column, columnIndex) => (
                    <div key={columnIndex} className="text-gray-300">
                      <a href={`/#bantuan/${column}`}>{column}</a>
                    </div>
                  ))}
                </div>
              )}
